...
import { Loader2, Eye, Search, Shield } from 'lucide-react';
      ...
      <div className="mx-auto max-w-md">
        <div className="flex flex-col items-center gap-6">
          <motion.div animate={{ rotate: 360 }} transition={{ duration: 2, repeat: Infinity, ease: 'linear' }}
            className="flex size-16 items-center justify-center rounded-2xl bg-redblack-600/10 glow-red">
            <Loader2 className="size-7 text-redblack-400" />
          </motion.div>

          <div className="text-center">
            <h3 className="text-lg font-semibold text-white font-heading tracking-wider">راہنما (REHNUMA) IS ANALYZING</h3>
            <p className="text-sm text-deep-400 mt-1">Scanning Pakistani stores for the best prices...</p>
          </div>
              ...
              return (
                <motion.div key={i} initial={{ opacity: 0, x: -10 }}
                  ...
                  className={cn(
                    ...
                    isActive ? 'bg-redblack-600/10 border border-redblack-500/20' : isDone ? 'bg-emerald-500/10' : 'bg-white/[0.02]'
                  )}>
                  <div className={cn(
                    'flex size-8 shrink-0 items-center justify-center rounded-lg transition-colors',
                    isActive ? 'bg-redblack-600 text-white' : isDone ? 'bg-emerald-600 text-white' : 'bg-white/5 text-deep-500'
                  )}>
                    {isDone ? (
                  ...
                  <span className={cn('text-sm leading-snug',
                    isActive ? 'text-redblack-300 font-medium' : isDone ? 'text-emerald-400' : 'text-deep-500'
                  )}>{step.label}</span>
                </motion.div>
              );
            })}
