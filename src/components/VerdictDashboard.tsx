...
const CONFIDENCE_CONFIG = {
  ...
  low: { label: 'Low Confidence — Estimated', color: 'text-deep-400', bg: 'bg-white/5', border: 'border-white/10' },
};
      ...
      <div className="mx-auto max-w-4xl">
        <div className="glass-card rounded-2xl p-6 md:p-8">
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-[11px] font-medium text-deep-400">
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-white/5 px-2.5 py-1 text-[11px] font-medium text-deep-400 border border-white/10">
              <Database className="size-3" /> {SOURCE_LABELS[dataSource] || dataSource}
            </span>
            {webPriceCount > 0 && (
              <span className="text-[11px] text-deep-500 border border-white/5 rounded-full px-2.5 py-1">
          ...
          {exactModel && (
            <div className="mb-4 px-3 py-2 rounded-lg bg-redblack-600/5 border border-redblack-500/10 text-xs text-redblack-300 font-mono">
              Detected: {exactModel}
            </div>
          )}
          ...
          <div className="grid gap-8 md:grid-cols-[1fr_1.5fr]">
            ...
            <div className="space-y-5">
              <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
                <h4 className="text-xs font-semibold uppercase tracking-wider text-deep-500 mb-3 flex items-center gap-1.5">
                  <TrendingDown className="size-3.5" /> Price Comparison
                </h4>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <span className="text-[11px] font-medium text-deep-500 uppercase tracking-wider">Your Price</span>
                    <p className="text-xl font-bold text-white mt-0.5 font-mono">{formatPKR(askingPrice)}</p>
                  </div>
                  <div>
                    <span className="text-[11px] font-medium text-deep-500 uppercase tracking-wider">Web Average</span>
                ...
                {savingsAmount > 0 ? (
                  ...
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.6 }}
                    className="mt-3 flex items-center gap-2 rounded-lg bg-redblack-600/10 border border-redblack-500/20 px-3 py-2">
                    <span className="text-lg">🔴</span>
                    <div>
                      <p className="text-sm font-semibold text-redblack-400">Overpriced by {formatPKR(-savingsAmount)}</p>
                      <p className="text-[11px] text-redblack-500">{Math.abs(savingsPercentage)}% above web average</p>
                    </div>
                  </motion.div>
                ) : null}
              </div>
              ...
              <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  <span className="inline-flex items-center gap-1 rounded-full bg-redblack-600/10 px-3 py-1 text-xs font-medium text-redblack-400 border border-redblack-500/20">
                    <Tag className="size-3" /> {brand}
                  </span>
                  <span className="inline-flex items-center gap-1 rounded-full bg-white/5 px-3 py-1 text-xs font-medium text-deep-300 border border-white/5">
                    <ShoppingCart className="size-3" /> {category}
                  </span>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {features.map((f) => (
                    <span key={f} className="rounded-md bg-white/5 px-2 py-0.5 text-[11px] font-medium text-deep-400 border border-white/5">{f}</span>
                  ))}
                </div>
              </div>
            </div>
